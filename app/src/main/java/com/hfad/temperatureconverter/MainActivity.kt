...
fun EnterTemperature(temperature: String, changed: (String) -> Unit) {
    TextField(
        value = temperature, label = { Text("Enter a temperature in Celsius") },
        onValueChange = changed, modifier =
...
fun Header(image: Int, description: String) {
    Image(
        painter = painterResource(image), contentDescription = description, modifier = Modifier
            .height(180.dp)
...
fun MainActivityContent() {
    ...
    Column(modifier = Modifier.padding(16.dp).fillMaxWidth()) {
        Header(R.mipmap.sunrise, "sunrise image")
